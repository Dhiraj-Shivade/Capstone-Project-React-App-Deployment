...
if [ "$GIT_BRANCH" == "origin/dev" ]; then
    # Tag the Built Image
    docker tag my-react-app:latest $DOCKER_DEV_REPO:v1

    ## For dev branch
    docker push $DOCKER_DEV_REPO:v1
    ...
    if git log --format=%B --grep="Merge branch 'dev'"; then
        # Tag the Built Image
        docker tag my-react-app:latest $DOCKER_PROD_REPO:v1

        # For main branch (after merging dev into main)
        docker push $DOCKER_PROD_REPO:v1
